...
handle_cast({delta_send, From, {Id, Type, _Metadata, Deltas}, Counter},
    ...
    lager:error("LASPVIN Received delta From=~p at TimeStamp=~p Took=~p microseconds DeltaVal:~p ~n", [From, time_stamp(), Time, DeltaVal]),
               ...
               false ->
                   case lists:member([Peer], lists:nth(1, ets:match(find_sub, {'_', Id, '$1', '_'}))) of
                       ...
                       false ->
                           ...
                           ets:insert(control_batch_find_sub, {lasp_support:mynode(), "c1", Id, NewHop, Peer})
                    end
            end
      end
   end,
    ...
    lists:foreach(fun(ReqRate) ->
                          case From == lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 3)) of
                             ...
                             false ->
                                ...
                                ets:insert(control_batch_find_sub, {lasp_support:mynode(), lists:nth(1,ReqRate), lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 2)), (lists:nth(1,ets:lookup_element(find_sub, lists:nth(1, ReqRate), 4))+1), From})
                          end
                        end,
