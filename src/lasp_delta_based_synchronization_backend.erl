...
extract_log_type_and_payload({rate_subscribe, Node, Rate}) ->
    [{delta_send_protocol, {Node, Rate}}];
extract_log_type_and_payload({rate_subscribe_ack, Node, Rate}) ->
    [{delta_send_protocol, {Node, Rate}}];
extract_log_type_and_payload({rate_refresh, Node}) ->
...
handle_cast({check_tonode_ack, ToNode, Act, From}, #state{store=Store}=State) ->
    ...
    case Act > 90000 of
        true ->
            lager:error("Received Don't Act for check_tonode_ack ~n");
        false ->
            [{TempToNode, TempHop, TempId, TempVia, TempFrom}] = ets:lookup(temp_tonode, ToNode),
            case lists:member([TempToNode], ets:match(c1, {"pseudopeer", '$1', '_', '_', '_'})) of
                true ->
                    [DelPseudoPeer] = ets:match_object(c1, {"pseudopeer", TempToNode, '_', '_', '_'}),
                    {_, _, _, Fsub, FId} = DelPseudoPeer,
                    ets:delete_object(c1, DelPseudoPeer),
                    send_lock(TempId, TempToNode, TempVia, TempHop, TempFrom),
                    case Fsub == ets:lookup_element(peer_rates, "subscription", 2) of
                        true ->
                            case lists:member([Fsub], ets:match(c1, {"pseudopeer", '_', '_', '$1', '_'})) of
                                true -> ok;
                                false ->
...
handle_info(batched_control_msgs, #state{store=Store}=State) ->
    lager:debug("LASPVIN Store: ~p State:~p ~n", [Store, State]),
    case ets:first(control_batch_find_sub) of
       '$end_of_table' -> ok;
       _Else ->
          ...
          lists:foreach(
              fun(fs_msg) ->
                 ...
                 {_, From, Rate, Id, Hop, Peer} = fs_msg,
                ...
                end, ets:tab2list(control_batch_find_sub))
    end,
    case ets:first(control_batch_find_sub_aq) == '$end_of_table' of
       true -> ok;
       false ->
          ...
          lists:foreach(
              fun(batch_msg) ->
                  ...
                  {_, Id, ToNode, Via, From, Hop, Peer} = batch_msg,
                ...
                end, ets:tab2list(control_batch_find_sub_aq))
    end,
    schedule_batched_control_messages(),
    {noreply, State};
               ...
               false ->
                   case lists:member([Peer], lists:nth(1, ets:match(find_sub, {'_', Id, '$1', '_'}))) of
                       ...
                       false ->
                           ...
                           ets:insert(control_batch_find_sub, {"find_sub", lasp_support:mynode(), "c1", Id, NewHop, Peer})
                    end
            end
      end
   end,
                ...
                true -> 
                    case Hop < lists:nth(1,lists:nth(1,ets:match(find_sub, {'_', Id, '_', '$1'}))) of
                        true -> 
                            ...
                            [DelSub] = ets:match_object(find_sub, {'_', Id, '_', '_'}),
                            ets:delete_object(find_sub, DelSub),
                            insert_findSub(ReqRate, Id, From, Hop),
...
forward_find_sub_on_join(From) ->
    ...
    lists:foreach(fun(ReqRate) ->
                          case From == lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 3)) of
                             true -> ok;
                             false ->
                                lager:error("LASPVIN sent find_sub on join req Id:~p, Hop:~p ~n", [ lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 2)), (lists:nth(1,ets:lookup_element(find_sub, lists:nth(1, ReqRate), 4))+1)]),
                                %?SYNC_BACKEND:send(?MODULE, {find_sub, lasp_support:mynode(), lists:nth(1,ReqRate), lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 2)), (lists:nth(1,ets:lookup_element(find_sub, lists:nth(1, ReqRate), 4))+1)}, From),
                                ets:insert(control_batch_find_sub, {"find_sub", lasp_support:mynode(), lists:nth(1,ReqRate), lists:nth(1,ets:lookup_element(find_sub, lists:nth(1,ReqRate), 2)), (lists:nth(1,ets:lookup_element(find_sub, lists:nth(1, ReqRate), 4))+1), From})
                          end
                        end,
                ...
                true -> 
                    ...
                    case Hop < lists:nth(1,lists:nth(1,ets:match(find_sub, {'_', Id, '_', '$1'}))) of
                        true -> 
                            ...
                            ets:insert(control_batch_find_sub_aq, {"find_sub_aq", Id, ToNode, Via, lasp_support:mynode(), Hop, lists:nth(1, lists:nth(1,ets:match(find_sub, {'_', Id, '$1', '_'})))});
                        false ->
                                ...
                                true ->
                                    case From == lasp_support:mynode() of
                                        true ->
                                            ...
                                            case ets:member(control_batch, "find_sub_aq") of
                                                true-> 
                                                    case ets:match(control_batch, {"find_sub_aq", Id, ToNode, '_', lasp_support:mynode(), Hop+1}) of
                                                        true -> ok;
                                                        false ->
                                                            ets:insert(control_batch_find_sub_aq, {"find_sub_aq", Id, ToNode, From, lasp_support:mynode(), Hop+1, lists:nth(1, lists:nth(1,ets:match(find_sub, {'_', Id, '$1', '_'})))})
                                                    end;
                                                false ->
                            ...
                            case lists:member([ets:lookup_element(peer_rates, "subscription", 2)], ets:match(find_sub_aq, {'_', ToNode, '$1', '_'})) of
                                true ->
                                    ...
                                    case From == ets:lookup_element(peer_rates, "subscription", 2) of
                                        ...
                                        false ->
                                            lager:error("Lower hop checking with Subscription ToNode ~p ~n", [ToNode]),
                                            check_with_subscription(ToNode, Hop, Id, Via, From)
                                    end,
                                    case ets:member(find_sub_aq, Id) of
                                        true ->
...
check_with_subscription(ToNode, Hop, Id, Via, From) ->
    case ets:member(temp_tonode, ToNode) of
        true ->
            case ets:lookup_element(temp_tonode, ToNode, 2) > Hop of
                true ->
